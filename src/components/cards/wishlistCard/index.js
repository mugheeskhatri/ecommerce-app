...
const Index = (props) => {
  ...
  useEffect(() => {
    axios.get(`${API}/product/get/single/${props.data}`).then(async (res) => {
      await setProductData(res.data);
      if (
        res.data.saleId === "" ||
        res.data.saleId === undefined ||
        res.data.saleId === null
      ) {
        axios
        ...
        axios
          .get(`${API}/flashSale/get/byId/${res.data.saleId}`)
          .then(async (res) => {
            if (res.status === 204) {
          ...
          <View style={{ width: "35%" }}>
            {productData?.image ? (
              <Image
                ...
                source={{ uri: IMAGE_URL + productData.image[0].src }}
              />
            ) : null}
          </View>
                ...
                >
                  ...
                  {discountType === "Percentage"
                    ? productData?.regularPrice -
                      Number((productData.regularPrice / 100) * discount)
                    : productData?.regularPrice - discount}
                </Text>
              </View>
            )}
          </View>
