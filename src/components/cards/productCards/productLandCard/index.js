...
const Index = (props) => {
  ...
  useEffect(() => {
    if (
      props.data.saleId !== "" &&
      props.data.saleId !== null &&
      props.data.saleId !== undefined
    ) {
      axios
        .get(`${API}/flashSale/get/byId/${props.data.saleId}`)
        .then((res) => {
          ...
          setAmountWithDiscount(
            res.data.discountType === "Percentage"
              ? props?.data?.regularPrice -
                  (props?.data?.regularPrice / 100) * res.data.percentOrAmount
              : props?.data?.regularPrice - res.data.percentOrAmount
          );
        });
    } else {
      setDiscountType("");
  ...
  useEffect(() => {
    const data = myCart?.filter(
      (product) => product?.productId === props.data?._id
    );
    if (data.length >= 1) {
      setAlready(true);
    }
  ...
  const addToCart = async () => {
    if (already) {
      const filteredData = myCart?.filter(
        (product) => product?.productId !== props.data?._id
      );
  ...
  useEffect(() => {
    if (props.popular === true) {
      if (
        ...
        axios
          .get(`${API}/category/get/single/${props.data.categoryId}`)
          .then(async (res) => {
            if (res.data.saleId) {
              await axios
      ...
      } else if (props.data.saleId) {
        console.log(props.data.saleId,"Sale Idd")
        axios
          .get(`${API}/flashSale/get/byId/${props.data.saleId}`)
          .then(async (res) => {
            if (res.status === 204) {
  ...
  return (
    ...
    >
      <View style={{ width: "35%" }}>
        <Image
          style={styles.image}
          source={{ uri: IMAGE_URL + props.data.image[0].src }}
        />
      </View>
      <View
        style={{
                ...
                >
                  {`${appearance.currency}.${
                    discountType === "Percentage"
                      ? props?.data?.regularPrice -
                        (props?.data?.regularPrice / 100) * discount
                      : props?.data?.regularPrice - discount
                  }`}
                </Text>
              </View>
            ) : (
      ...
      <TouchableOpacity
        onPress={() => {
          if (already) {
            ...
            if (
              props.data.differentColors === " Yes" ||
              props.data.differentSizes === "Yes"
            ) {
      ...
      >
        {!already ? (
          <AntDesign name="plus" size={14} color={appearance.backgroundColor} />
        ) : (
          <AntDesign
            name="close"
            size={14}
      ...
      >
        {addedToWish ? (
          <AntDesign name="heart" size={16} color={appearance.secondaryColor} />
        ) : (
          <AntDesign
            name="hearto"
            size={20}
