...
const Index = ({ navigation }) => {
  const { appearance, myCart, setMyCart, discount, setDiscount } =
    useContext(AuthContext);
  const [totalAmount, setTotalAmount] = useState(0);
  const [totalAmounWithouDiscount, setTotalAmountWithoutDiscount] =
    useState("");
  ...
  const firstFunction = () => {
    ...
    for (var i = 0; i < myCart.length; i++) {
      price +=
        myCart[i].discountType === "Percentage"
          ? (Number(myCart[i].price) -
              Number(Number(myCart[i].price) / 100) *
                myCart[i].discountPercent) *
            Number(myCart[i].quantity)
          : (myCart[i].price - myCart[i].discountPercent) *
            Number(myCart[i].quantity);
  ...
  const changeCoupenText = (e) => {
    setCoupen(e);
    if (e.length === 6) {
      axios.get(`${API}/coupen/find/${e}`).then(async (res) => {
        if (res.status === 202) {
          setAvailable(true);
          if (res.data.allDeal === "true") {
            var afterCoupen =
              res.data.discountType === "Percentage"
                ? totalAmounWithouDiscount -
                  (totalAmounWithouDiscount / 100) * res.data.percentOrAmount
                : totalAmounWithouDiscount - res.data.percentOrAmount;
            setAmountDiscount(totalAmounWithouDiscount - afterCoupen);
          } else if (res.data.allDeal === "false") {
            var afterCoupen =
              res.data.discountType === "Percentage"
                ? ((totalAmounWithouDiscount - amountDiscount) / 100) *
                  res.data.percentOrAmount
                : res.data.percentOrAmount;
  ...
  const deleteFromCart = (index) => {
    AsyncStorage.getItem("ecommerce_cart").then(async (res) => {
      ...
      cart.splice(index, 1);
      AsyncStorage.setItem("ecommerce_cart", JSON.stringify(cart)).then(() => {
        setMyCart(cart);
      });
    });
  };
  console.log("Acart Type", appearance);
  const onRefresh = () => {
    firstFunction();
  };
  ...
  return (
    ...
    >
      {/* <RefreshControl onRefresh={onRefresh} style={{ width: "100%", flex: 1 }}> */}
      {myCart.length !== 0 ? (
        <View style={{ width: "100%", flex: 1 }}>
          <View style={{ width: "100%", flex: 1, paddingBottom: 115 }}>
            <RefreshControl>
              <ScrollView>
                <Header onBack={() => navigation.goBack()} title="My Cart" />
                {appearance?.cartCardType === "swipeCard" &&
                  myCart?.length !== 0 && (
                ...
                {appearance?.cartCardType === "normalCard" &&
                  myCart?.length !== 0 &&
                  myCart.map((v, i) => {
                    return (
                      <CartBorderCard
                ...
                {appearance?.cartCardType === "bigCard" &&
                  myCart?.length !== 0 &&
                  myCart.map((v, i) => {
                    return (
                      <CartBigImageCard
          ...
          >
            <View style={{ flexDirection: "row", marginTop: 10 }}>
              ...
              >
                ...
                >
                  {appearance.currency}.
                  {totalAmounWithouDiscount - amountDiscount}
                </Text>
              </View>
            </View>
            <View
              ...
              <ScrollView style={{ width: "100%" }}>
                  ...
                  >
                    <TextInput
                      ...
                      style={{
                        ...
                        paddingLeft: 10,
                        color: appearance.contrastBackgroundColor,
                      }}
                    />
                    <View
                      style={{
                    ...
                    >
                      ...
                      <AntDesign
                        name={available ? "checkcircleo" : "closecircleo"}
                        size={15}
                        color={appearance.contrastBackgroundColor}
                        style={{ marginLeft: 5 }}
                      />
                ...
                <View
                  style={{
                    ...
                    marginTop: 5,
                    paddingBottom: 10,
                    borderBottomColor: appearance.backgroundColor,
                    borderBottomWidth: 1,
                  }}
                >
                  <View style={{ width: "50%" }}>
