...
const Index = ({ navigation }) => {
  ...
  const { appearance, discountForThis, discountTypeForThis } =
  ...
  const _renderItem = ({ item, index }) => {
    return (
      ...
      >
        <Image
          style={{
            borderColor: appearance.contrastBackgroundColor,
            ...styles.image,
          }}
          source={{ uri: item.src }}
        />
  ...
  const { currentProduct, myCart, setMyCart, myWishlist, setMyWishList } =
    useContext(AuthContext);
  ...
  const [amountWithoutDiscount, setAmountWithoutDiscount] = useState(
    currentProduct.regularPrice * quantity
  );
  const [amountOfProduct, setAmountOfProduct] = useState(
    discountTypeForThis === "Percentage"
      ? quantity *
          (currentProduct.regularPrice -
            (currentProduct.regularPrice / 100) * discountForThis)
      : quantity * (currentProduct.regularPrice - discountForThis)
  );
  const [discountAmount, setDiscountAmount] = useState(
    discountTypeForThis === "Percentage"
      ? (amountWithoutDiscount / 100) * discountForThis
      : discountForThis * quantity
  );
  ...
  const addProduct = async () => {
    if (currentProduct.differentSizes === "Yes" && selectedSize === "") {
      ...
      const data = myCart?.filter(
        (product) => product?.productId === currentProduct?._id
      );
      if (data.length >= 1) {
        ...
        const cart = myCart;
        cart.unshift(form);
        await AsyncStorage.setItem("ecommerce_cart", JSON.stringify(cart)).then(
          () => {
  ...
  const addToWishlist = () => {
    ...
    setAddedToWish(true);
    AsyncStorage.setItem("ecommerce_wishlist", JSON.stringify(wish)).then(
      async (res) => {
        setMyWishList(wish);
      }
    );
  ...
  const deleteFromWish = () => {
    ...
    AsyncStorage.setItem("ecommerce_wishlist", JSON.stringify(filtered)).then(
      () => {
    ...
    >
      <View style={{ width: "100%", flex: 8 }}>
        <ScrollView style={{ width: "100%" }}>
          <Header onBack={() => navigation.goBack()} title="Product Detail" />
          ...
          <View style={{ width: "100%", paddingHorizontal: 20 }}>
            <View style={{ width: "100%", flexDirection: "row" }}>
              <View style={{ width: "87%", paddingTop: 5 }}>
                <Text
                  style={{ ...styles.name, color: appearance.primaryColor }}
                >
                  {currentProduct.name}
                </Text>
                <Text
                  style={{
                    color: appearance.primaryLightColor,
              ...
              <View
                style={{ width: "13%", alignItems: "flex-end", padding: 8 }}
              >
                <TouchableOpacity
                  onPress={() =>
                    addedToWish ? deleteFromWish() : addToWishlist()
                  }
                >
                  {addedToWish ? (
                    <AntDesign
            ...
            <View
              style={{
                ...
                marginTop: 10,
                paddingBottom: 10,
                borderBottomColor: appearance.contrastBackgroundColor,
                borderBottomWidth: 2,
              }}
            >
                  ...
                  >
                    {`${appearance.currency}.${
                      discountTypeForThis === "Percentage"
                        ? currentProduct.regularPrice -
                          (currentProduct.regularPrice / 100) * discountForThis
                        : currentProduct.regularPrice - discountForThis
                    }`}
                  </Text>
                  {discountForThis !== "" ? (
                    <View
                    ...
                    >
                      <View
                        style={{
                          borderWidth: 0.5,
                          borderColor: appearance.secondaryColor,
                          width: "100%",
                          position: "absolute",
                          top: 12,
                        }}
          ...
          <View style={{ width: "100%", flexDirection: "row", marginTop: 10 }}>
            <View
              style={{
                ...
                borderBottomWidth: selectedTab === "description" ? 1 : 0,
              }}
            >
          ...
          {selectedTab === "description" ? (
            <View
              style={{ width: "100%", paddingHorizontal: 20, marginTop: 10 }}
            >
              <Text
                style={{
                  fontSize: 14,
          ...
          >
            <View
              style={{
                ...
                justifyContent: "center",
                backgroundColor: appearance.secondaryColor,
                backgroundColor: appearance.contrastBackgroundColor,
                borderRadius: 20,
                flexDirection: "row",
              }}
            >
                    ...
                    setAmountOfProduct(
                      quantity === 1
                        ...
                        ? amountOfProduct -
                          Number(
                            currentProduct.regularPrice -
                              (currentProduct.regularPrice / 100) *
                                discountForThis
                          )
                        : amountOfProduct -
                          Number(currentProduct.regularPrice - discountForThis)
                    );
                  }}
                >
                  <MaterialIcons
                    ...
                    setDiscountAmount(
                      discountTypeForThis === "Percentage"
                        ? discountAmount +
                            (currentProduct.regularPrice / 100) *
                              discountForThis
                        : Number(discountAmount) + Number(discountForThis)
                    );
                    setAmountOfProduct(
                      amountOfProduct +
                        Number(
