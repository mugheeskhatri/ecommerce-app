...
const Index = ({ navigation }) => {
  ...
  const verifyOTP = () => {
    ...
    axios.post(`${API}/user/verify-otp`, form).then((res) => {
      if (res.status === 205) {
  ...
  const resendOtp = () => {
    ...
    axios.post(`${API}/user/resend-otp`, form).then((res) => {
      if (res.status === 201) {
        Alert.alert(
          "OTP Sent Again",
          `OTP has been sent again to ${userEmail}`
        );
      }
    });
  };
      ...
      <KeyboardAvoidingView style={{ width: "100%", flex: 1 }}>
        <View style={styles.logoSection}>
          <Image
            ...
            source={{ uri: IMAGE_URL + appearance.logo }}
          />
        </View>
        <View
          style={{
            ...
            >
              ...
              >
                <TouchableOpacity onPress={resendOtp}>
                  <Text style={{ color: appearance.secondaryColor }}>
                    RESEND OTP
                  </Text>
                </TouchableOpacity>
              </View>
