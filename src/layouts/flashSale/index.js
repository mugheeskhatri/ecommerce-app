...
const Index = (props) => {
  ...
  useEffect(() => {
    axios.get(`${API}/flashSale/get`).then(async (res) => {
      var startDate = await new Date(res.data[0].startDate);
      const date = new Date(res.data[0].endDate);
      const checkAvailability = currentDate.getTime() - date.getTime() < 1;
      ...
      {available ? (
        <View style={{ width: "100%" }}>
          <View style={styles.headingSection}>
            ...
            >
              <View style={{ marginRight: 10, justifyContent: "center" }}>
                <Text
                  style={{
                    fontFamily: "Monstret_reg",
                    fontSize: appearance.lightFont,
              ...
              <View>
                <CountDown
                  size={12}
                  until={
                    Number(saleEndDate.getTime() - currentDate.getTime()) / 1000
                  }
                  digitStyle={{
          ...
          >
            ...
            <ViewMore onClick={props.onClick} />
          </ScrollView>
        </View>
      ) : null}
    </View>
